        ...
        for keys in self.__keys__:
            for key in keys:
                ...
                if isinstance(val, int) or isinstance(val, int):
                    ...
                    val = b([bee for bee in str(val) if bee != 0 and bee != "" and bee != "0"])
...
class SectionStructure(Structure):
    ...
    def get_hash_sha256(self):
        ...
        if sha256 is not None:
            results = list(bytearray(self.get_data()))
            js.globals["dojshash"] = results
            jshash = js.eval("DoHashFunction(dojshash, 'sha2')")
            return jshash
    ...
    def get_hash_md5(self):
        ...
        if md5 is not None:
            results = list(bytearray(self.get_data()))
            js.globals["dojshash"] = results
            jshash = js.eval("DoHashFunction(dojshash, 'md5')")
            return jshash
...
class PE(object):
    ...
    def __parse__(self, fname, data, fast_load):
        ...
        if not self.DOS_HEADER or self.DOS_HEADER.e_magic != IMAGE_DOS_SIGNATURE:
            print(self.DOS_HEADER.e_magic)
            print(IMAGE_DOS_SIGNATURE)
            print(dos_header_data)
            print(self.DOS_HEADER)
            raise PEFormatError(dos_header_data)
